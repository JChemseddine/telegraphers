...
def get_args():
    parser = argparse.ArgumentParser(
        description="Train Radial Kac–CDF model on CIFAR-10 with configurable schedules."
    )
    # Training settings
    ...
    # Kac process params
    parser.add_argument('--a',               type=float, default=9.0, help="Jump rate a")
    parser.add_argument('--c',               type=float, default=3.0,   help="Jump size c")
    ...
    parser.add_argument('--eval_interval_fid', type=int, default=5_000)
    parser.add_argument('--num_samples',       type=int, default=100)
    parser.add_argument('--fid_num_real',      type=int, default=10000)
    parser.add_argument('--fid_image_size',    type=int, default=75)
    parser.add_argument('--save_dir',          type=str, default='results_cdf_cifar10')
...
def main():
    ...
    wandb.init(
        entity="Appliedmathlab",          # <— add this
        project="Kac-CIFAR10",
        name=f"a{args.a}_c{args.c}_T{args.T}_{args.schedule}",
        config=vars(args),
    ...
    ema  = AveragedModel(unet, multi_avg_fn=get_ema_multi_avg_fn(0.99))
    opt  = optim.Adam(unet.parameters(),lr=args.lr)
    sampler=TorchKacConstantSampler(a=args.a, c=args.c, T=args.T, M=int(50000*args.T), K=4096)
        # Prepare fixed noise
    with torch.no_grad():
        # For visualization
        tT_vis = torch.ones(args.num_samples,1,device=device) * args.T
        tauT_vis = sampler.sample(tT_vis.squeeze(1), dim=dim).to(device)
        xT_fixed_vis = tauT_vis
        # For FID evaluation
        tT_fid = torch.ones(args.fid_num_real,1,device=device) * args.T
        tauT_fid = sampler.sample(tT_fid.squeeze(1), dim=dim).to(device)
        xT_fixed_fid = tauT_fid
