...
def get_args():
    ...
    parser.add_argument('--save_dir',          type=str, default='results_diff_cifar')
...
def main():
    ...
    # Init W&B
    wandb.init(
        project="DiffFM-CIFAR10",
        name=f"sigma{args.sigma}_T{args.T}_{args.schedule}",  # your custom run name
        config=vars(args),
    ...
    for step in progress:
        ...
        # Replace Kac sampling with:
        eps = torch.randn_like(x0)
        xt = get_f(t.squeeze(1), args.T, args.schedule).unsqueeze(1) * x0 + sigma * torch.sqrt((t**2).clamp(min=1e-8)) * eps

        # Replace compute_velocity call with:
        velocity = (xt - get_f(t.squeeze(1), args.T, args.schedule).unsqueeze(1) * x0) / ( t.clamp(min=1e-8))
        drift    = get_df(t.squeeze(1), args.T, args.schedule).unsqueeze(1) * x0

        # Final target:
        target = eps-x0#velocity + drift
        # Prediction
