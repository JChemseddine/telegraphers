...
class TorchKacConstantSampler:
    ...
    """
    def __init__(self, a: float, c: float, T: float, M: int, K: int = 1024,
                 device=None, dtype=torch.float32):
        self.a, self.c = a, c
        self.beta = a / c
        self.T, self.M, self.K = T, M, K
        self.device = device or torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.dtype = dtype

        # Build forward tables (same as before)
        ...
        for j, t in enumerate(t_grid):
            ...
            ratio = np.where(z > 1e-6, i1e(z) / z,
                              0.5 + (z * z) / 16.0 + (z ** 4) / 384.0)
        ...
        # Build inverse-CDF lookup for continuous part
    ...
    def sample(self, t: Tensor, dim: int = 1) -> Tensor:
        """
        Sample Kac displacements at times t (shape [...]), output shape [..., dim].
        Mixes atomic ±c*t (prob=e^{-a t}) with continuous part.
        """
        orig_shape = t.shape
