...
def get_args():
    ...
    parser.add_argument('--eval_interval_fid', type=int, default=10000)
    parser.add_argument('--num_samples', type=int, default=100)
    parser.add_argument('--fid_num_real', type=int, default=2000)
    parser.add_argument('--fid_image_size', type=int, default=32)

    # optimisation niceties
    parser.add_argument('--scheduler', type=str, default='None',
                        choices=['None', 'cos', 'polynomial'])
...
def get_UNET(img_size=32, channels=3, size="small", dropout=0.0):
    if size == "small":
        return UNetModel(
...
class time_wrapper(torch.nn.Module):
...
class ODEWrapper(torch.nn.Module):
    ...
    def forward(self, t, x):
        x_in = x.view(-1, 3, 32, 32)
        t_exp = torch.full((x.shape[0],), t.item(), device=x.device)
        
        return self.fmap(x_in, t_exp).view(x.shape)

                
def sampling(fmap, x_T, T, num_steps, device='cuda', max_batch=500, method='euler'):
    ...
    with torch.no_grad():
        for chunk in chunks:
            ...
            x_traj_chunk = odeint(ode_func, chunk, t_vals, method=method)
...
def get_f(t, T, name, net=None):
    ...
    if name == 'exp':
        ...
        y_raw = torch.sigmoid(-net(t.unsqueeze(1) / T)).flatten()
        # anchor values at the boundaries         (use same device as t)
        y0 = torch.sigmoid(-net(torch.tensor([0.0], device=t.device) / T)).item()
        y1 = torch.sigmoid(-net(torch.tensor([T],    device=t.device) / T)).item()
...
def main():
    ...
    wandb.init(project="Kac-Flow-Radial-CIFAR10", config=vars(args))
    ...
    cifar = datasets.CIFAR10('./data', train=True, download=True,
                             transform=transform)
    loader      = DataLoader(cifar, batch_size=args.batch_size, shuffle=True,
                             num_workers=4, drop_last=True)
    fid_loader  = DataLoader(cifar, batch_size=args.batch_size*4,
    ...
    if args.scheduler == 'cos':
        ...
        use_sched = True
        sched_kac = torch.optim.lr_scheduler.PolynomialLR(
            opt, total_iters=args.epochs - args.warm_up, power=1, 
        )
    if args.warm_up > 0:
        warm_kac = torch.optim.lr_scheduler.LinearLR(
            opt, start_factor=1e-8/ args.lr_kac, end_factor=1.0, total_iters=args.warm_up
        )
    ...
    for step in progress:
        ...
        x0 = x0.to(device).view(-1, 3, 32, 32)        # (B,1,28,28)
        ...
        # exact Kac jump
        u  = torch.randn(B, 3*dim, device=device)
        ...
        x0_flat = x0.view(B, 3*dim)
        xt_flat = f_t * x0_flat + noise_disp
        xt      = xt_flat.view(B, 3, 32, 32)
        ...
        x0_flat = x0.view(B, 3*dim)
        xt_flat = xt.view(B, 3*dim)
        drift   = get_df(t, args.T, args.f, net_scheduler) * x0_flat#(-1.0 / args.T) * x0_flat

        # --------------- training step ---------------
        net.train()
        pred_velo = net(xt, t_vec).view(B, 3*dim)
        ...
        w = 1. / (target_velo.norm(dim=1, keepdim=True)**2 + 1e-4)
        loss = (w * (pred_velo - target_velo).pow(2)).mean()
        ...
        wandb.log({'loss': loss.item(), 'lr': opt.param_groups[0]['lr']})
        ...
        if (step + 1) % args.eval_interval == 0:
            ...
            with torch.no_grad():
                ...
                traj = sampling(ema, xT_flat, args.T, num_steps=100,
                                device=device)
                x_gen = traj[-1].view(-1, 3, 32, 32).cpu()

                # save sample grid
                grid = make_grid(x_gen[:64], nrow=8,
                ...
                plt.tight_layout()
                sample_path = os.path.join(save_dir,
                                           f'samples_step_dorpri5_{step+1}.png')
                plt.savefig(sample_path)
                wandb.log({"samples dopri5": wandb.Image(plt)})
                plt.close()

                traj = sampling(ema, xT_flat_fixed, args.T, num_steps=100,
                                device=device,)
                x_gen = traj[-1].view(-1, 3, 32, 32).cpu()
                ...
                plt.tight_layout()
                sample_path = os.path.join(save_dir,
                                           f'samples_step_fix_{step+1}.png')
                plt.savefig(sample_path)
                wandb.log({"samples fix": wandb.Image(plt)})
                plt.close()

            # optional quick FID every 10 evals
        if (step + 1) % (args.eval_interval_fid) == 0:
                with torch.no_grad():
                    traj = sampling(ema, xT_flat_fixed, args.T, num_steps=1000,
                                device=device,method='rk4')
                    x_gen = traj[-1].view(-1, 3, 32, 32)
                    ...
                    plt.tight_layout()
                    sample_path = os.path.join(save_dir,
                                            f'samples_step_fix_rk4_{step+1}.png')
                    plt.savefig(sample_path)
                    wandb.log({"samples fix rk4": wandb.Image(plt)})
                    plt.close()
                    ...
                    tT_vec = tT.squeeze(1)
                    uT = torch.randn(args.fid_num_real, 3*dim, device=device)
                    uT /= uT.norm(dim=1, keepdim=True)
                    s_kT, sums_T = create_sk(args.a, tT_vec, args.T)
                    ...
                    traj = sampling(ema, xT_flat, args.T, 100,
                                    device=device)
                    fakes = traj[-1].view(args.fid_num_real, 3, 32, 32)
                    ...
                    torch.save(net.state_dict(), os.path.join(save_dir, f'net_{step+1}.pt'))
