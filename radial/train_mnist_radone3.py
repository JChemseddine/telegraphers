...
def main():
    ...
    # ----------- model & opt ---------------
    net = get_UNET(size=args.unet_size).to(device)

    
    if args.f == 'net':
        net_scheduler = net_s.NetScheduler(num_hidden_layers=1024)
    else:
        net_scheduler =  None
    ...
    for step in progress:
        ...
        if (step + 1) % args.eval_interval == 0:
            ...
            with torch.no_grad():
                ...
                traj = sampling(net, xT_flat, args.T, num_steps=100,
                                device=device)
            ...
            if (step + 1) % (args.eval_interval * 10) == 0:
                with torch.no_grad():
                    ...
                    tauT = sample_trj(tT_vec, s_kT, sums_T,
                                      args.a, args.T)
                    xT_flat = (args.c * tauT).unsqueeze(1) * uT
                    traj = sampling(net, xT_flat, args.T, 100,
                                    device=device)
