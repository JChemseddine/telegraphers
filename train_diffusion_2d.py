...
def get_args():
    ...
    parser.add_argument('--results_dir',   type=str,   default="results2d_diff")
    parser.add_argument('--eps',           type=float, default=1e-15)
    parser.add_argument('--seed',          type=int,   default=1)
    return parser.parse_args()


def visualize_and_save(wrapper, gmm, sigma, T, output_dir,
                       num_steps=50, num_samples=5000, dim=2,
                       epsilon=0.0, device='cuda'):
    from matplotlib import animation

    with torch.no_grad():
        x0 = gmm.sample((num_samples,)).to(device)
        eps_noise = torch.randn_like(x0)
        xT = x0 + sigma * np.sqrt(T) * eps_noise
    ...
    frames = list(range(num_steps)) + [num_steps - 1] * 20
...
def main():
    ...
    args.results_dir = f"results2d/results_diff"
    subdir = f"sigma{args.sigma}_T{args.T}_{args.eps}"
    ...
    # Build GMM on grid [-1,0,1]^2
    grid = list(itertools.product([-1.0, 0.0, 1.0], repeat=dim))
    means = torch.tensor(grid, device=device)
    ...
    for step in tqdm(range(args.ntrain), desc="Training"):
        ...
        eps_noise = torch.randn_like(x0)
        xt = x0 + sigma * torch.sqrt(t.clamp(min=eps)) * eps_noise

        velo_target = (xt - x0) / (2 * t.clamp(min=eps))
        ...
        if (step + 1) % args.val_interval == 0:
            with torch.no_grad():
                ...
                xT = x0_val + sigma * np.sqrt(T) * eps_val

                ode_func = ODEWrapper(wrapper).to(device)
                t_vals = torch.linspace(T, eps, args.num_steps_eval, device=device)
    ...
    # Load best model and generate GIF
    model.load_state_dict(torch.load(os.path.join(output_dir, 'best_model.pt')))
    visualize_and_save(wrapper, gmm, sigma, T, output_dir,
                       num_steps=50, num_samples=args.val_samples,
                       dim=dim, epsilon=eps, device=device)
